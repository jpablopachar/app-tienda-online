...
const authGuard = (): Observable<boolean | UrlTree> => {
  ...
  return store.select(fromUser.selectUser).pipe(
    ...
    tap((state: UserState): void => {
      if (!state.id) router.navigate(['auth/login']);
    }),
    map((state: UserState): boolean => !!state.id)
  );
};

export const authGuardCanActivate = authGuard;
export const authGuardCanActivateChild = authGuard;
export const authGuardCanMatch = authGuard;
